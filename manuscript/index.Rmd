...
We have collected `r sum(tract$nListings)` Seattle rental housing listings from Craigslist over the course of our data collection (`r min(tract$listingQtr)` to `r max(tract$listingQtr)`). Of the overall corpus of listings we've collected, `r sum(tract$n1B)` (about `r round(sum(tract$n1B)/sum(tract$nListings), 2)*100`%) were one-bedroom listings. 

The following series of graphics summarize the listing data collected to date and compare rent estimates from these data to ACS data and Dupre+Scott. Wherever possible, visualizations focus on one-bedroom units to make comparisons between data for the most common unit type in Seattle. However, for Figure 2 data limitations require that the comparison is all housing units.

<br>
<hr>
<br>

Figure 1 shows a reference map of Seattle. In the right pane, each point indicates a one-bedroom listing collected at some point over our study's span. The polygons overlaid on higher-density neighborhoods around Downtown, South Lake Union and Ballard correspond to particularly strong Craigslist listing activity.
...
Figure 3 is a line graphic indicating the Seattle-wide median rent for all sizes of rental units, with the longest set of data (Dupre+Scott) reaching back to 1997 to show long-term dynamics of Seattle rental housing costs. The first takeaway from the graphic is the large spike in rent levels following 2010, with a similar trajectory followed in the estimates from Dupre+Scott and the ACS.
