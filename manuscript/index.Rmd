...
The key steps of any data process can be distilled into three tasks: a.) _extract_ a table of interest from a specified origin source, b.) _transform_ it to support the desire analyses and c.) _load_ the transformed data to a storage destination for future use. In the present case, a program extracts scraped listings observed since the database 
<br>
