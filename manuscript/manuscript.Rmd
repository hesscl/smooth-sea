...
output:
  pdf_document:
    df_print: paged
abstract: Online classified ads are a key source of information about local rental
  housing markets. This research first assesses the agreement of census tract rent
  estimates based on scraped rental advertisements with existing data sources. The
  second component of the paper investigates temporal and spatial dynamics in one-bedroom
  rents among Seattle neighborhoods using spatial models. Posterior distributions
  of all model hyperparameters and parameters are estimated with integrated nested
  Laplace approximations (INLA). Training data for models cover quarterly tract estimates
  for Q2 2017 through Q1 2018, while the test data are Q2 2018 median values as observed.
  The top model assumes that spatial and temporal dynamics are separable, i.e. independent
  of each other, and non-interactive. This spatial random-effects model with AR(1)
  temporal structure achieves a mean absolute error (MAE) of about $100 for the training
  data and $140 for the test data.
---
...
[Helena](http://helena-lang.org) is a high-level programming language aimed at helping end users automate web scraping as a method of data collection. Users write programs in an extension for Google Chrome.  To write a new program, they record themselves collecting the first row of the data they want to collect; they use standard browser interactions -- clicking, typing, and interacting with webpage widgets, and they highlight content they want to scrape.  The Helena Chrome extension translates this input demonstration into a scraping program written in the Helena language, which a user can run, edit, or schedule for later runs. Although Helena programs collect data at roughly the speed of the user demonstration, users can run Helena programs in parallel to increase the rate of data collection. Collected data is stored on a remote server, from which users can later download either the full table of scraped data or a subset scraped within a specified window of time. 

Helena learns how to find data on a webpage using features of the user-facing webpage interface rather than relying on the stability of the underlying document object model (DOM) structure. Thus, even if the HTML code of a site changes frequently over the course of longitudinal data scraping, as long as the website limits overhauls of the features users see, Helena can often adapt to webpage redesigns. Helena also uses a language construct called the skip block to avoid re-scraping already-collected items. These features make Helena programs robust to issues that commonly plague traditional web scraping workflows. Overall, Helena's demonstration-based method reduces the time needed to build a scraper from scratch, giving social scientists the capacity to quickly scrape a single dataset or to build ongoing data collection processes.

For the UW team's data collection, a Helena program runs 24 hours a day to scrape rental listings from the Seattle Craigslist. It is important to note that web scraping does violate Craigslist's Terms of Service and that we are using all scraped data exclusively for academic research. For a more thorough discussion of this issue, consult page 3 of [Boeing and Waddell (2016)](https://arxiv.org/ftp/arxiv/papers/1605/1605.05397.pdf).
...
Figure 2 is a heatmap for the time-series of one-bedroom listings per day. The gray cells indicate days where our scraper was offline. Outages were more common in the earliest phases of work, but now are relatively rare. 
...
The following models learn from Craigslist data accumulated from Q1 2017 Q1 through Q1 2018 to accomplish a few different goals. The most prominent goal is to use the panel data's temporal and spatial structure to understand variation in rent levels over time and across space. A secondary goal is to forecast the Seattle neighborhood rent surface for the current quarter. Finally, the following models allow for relatively straightforward imputation of missing values based on time period and the rent values of proximate tracts. 
...
#spatial random effect model (BYM 1991) with quarterly seasonal fixed-effect
...
ggplot(bym_df %>% filter(model == "E. Spatial AR(1)"), 
  ...
  scale_color_viridis_c(option = "A", labels = scales::dollar, direction = -1) +
  xlab("") +
  ylab("Median 1B Rent\n") +
...
obs_map_series <- ggplot(sea_f, aes(x = long, y = lat, group = group, fill = actualRent)) +
  ...
  scale_fill_viridis_c(labels = scales::dollar, na.value = "grey80",
                       limits = c(800, 2800), option = "A", direction = -1) +
  coord_quickmap() +
