...
#load demo extract
#demo <- read_csv(file = "./input/tractDemoSumStats.csv")

#load culture extract
#culture <- read_csv(file = "./input/tractCultureSumStats.csv")

#load police report extract
#police <- read_csv(file = "./input/tractPoliceReportSumStats.csv")
...
#panel <- left_join(panel, demo)
#panel <- left_join(panel, culture)
#panel <- left_join(panel, police)
#panel <- left_join(panel, sea_shp@data)
...
sea_df$nsar1_Eff <- m.nsar1$summary.random$idqtr[sea_df$idqtr, "0.5quant"] 
...
#spatio-temporal smoothing model, AR(1) process with linear space-time interaction (Bernadelli 1995)
form.spt <- log(med1B) ~ 1 +
  f(idtract, model = "bym", #ICAR spatial RE for tract neighbors + IID RE for tract
    scale.model = T,
    graph = "./output/seatract.graph") +
  f(idqtr, model = "ar", order = 1) +
...
obs <- sea_f %>%
  ...
  dplyr::select(GISJOIN, long, lat, group, rent, var, actualRent, spt_Med, NAMELSAD10, n1B)
...
comb <- bind_rows(obs, pred) %>%
  mutate(tooltip = paste0(NAMELSAD10, "<br>", "Forecast: $", round(spt_Med), "<br>", "Qtr-to-Date: $", round(actualRent), "<br>", "N Qtr-to-Date: ", n1B))
